<template>
  <v-row align="center" justify="center">
    <v-form ref="form" v-model="valid">
      ...
      <v-btn block :disabled="!valid" v-on:click="register()">
        Register
      </v-btn>
    </v-form>
  </v-row>
  ...
  data() {
    return {
      ...
      passwordRules: [
        ...
        (v) =>
          (v && v.length >= 6) || "Password must be 6 characters or longer",
      ],
    };
  },
  methods: {
    async register() {
      ...
      setTimeout(async () => {
        this.$socket.client.emit("userRegistered", {
